  ...
  <table class="table table-striped">
    ...
    foreach($list['rows'] as $rows){
    	...
    	foreach($header as $rows1){
            ...
            }elseif($rows1['name']=='file_path'){?>
<td><img src="<?=$base?><?=str_replace($folder_project,'',str_replace('/files/', '/thumbs/', $val))?>" class="mc_preload wp-post-image" style="display: block;"></td>
            <?php }else{
                switch ($rows1['type']) {
                    case 'list':
